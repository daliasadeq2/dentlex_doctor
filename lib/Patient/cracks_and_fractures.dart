...
class CracksAndFracturesScreen extends StatefulWidget {
  const CracksAndFracturesScreen({super.key});

  @override
  State<CracksAndFracturesScreen> createState() => _CracksAndFracturesScreenState();
}

class _CracksAndFracturesScreenState extends State<CracksAndFracturesScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = false;
  String activeFilter = 'التشققات والكسور';

  final LayerLink _cracksLayerLink = LayerLink();
  OverlayEntry? _cracksOverlayEntry;
  bool _isCracksMenuOpen = false;

  void _toggleCracksMenu() {
    if (_isCracksMenuOpen) {
      _closeCracksMenu();
    } else {
      _openCracksMenu();
    }
  ...
  void _openCracksMenu() {
    setState(() {
      _isCracksMenuOpen = true;
      activeFilter = 'التشققات والكسور';
    });
    _cracksOverlayEntry = _createCracksOverlayEntry();
    Overlay.of(context).insert(_cracksOverlayEntry!);
  }

  void _closeCracksMenu() {
    if (_cracksOverlayEntry != null) {
      _cracksOverlayEntry!.remove();
      _cracksOverlayEntry = null;
      setState(() {
        _isCracksMenuOpen = false;
      });
    }
  }

  OverlayEntry _createCracksOverlayEntry() {
    final List<String> cases = [
      'متلازمة السن المتشقق',
      'كسر في حدبة السن',
      'كسر عمودي في الجذر',
      'انقسام السن',
      'كسر التاج',
                          ...
                          builder: (context, setPopupState) {
                            ...
                            return InkWell(
                              onTap: () {
                                ...
                                setPopupState(() {});
                                _cracksOverlayEntry?.markNeedsBuild();
                              },
                              child: Container(
        ...
        child: Row(
          children: [
            ...
            CompositedTransformTarget(
              link: _cracksLayerLink,
              child: _buildCracksFilterButton('التشققات والكسور', activeFilter.contains('كسر') || activeFilter.contains('تشقق') || _isCracksMenuOpen),
            ),
            const SizedBox(width: 8),
            _buildFilterChip(
  ...
  Widget _buildCracksFilterButton(String label, bool isActive) {
    return GestureDetector(
      onTap: _toggleCracksMenu,
      child: Container(
        height: 40,
  ...
  Widget _buildFilterChip(String label, bool isActive, {VoidCallback? onTap}) {
    return GestureDetector(
      onTap: onTap ?? () {
        if (label == 'الترميمات') {
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const ImpactedTeethScreen()));
        } else if (label == 'زراعة الأسنان') {
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const MeasurementsDentalImplantsScreen()));
        } else {
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('رقم الطلب: 2458', style: TextStyle(color: Color(0xFF8A8AA3), fontSize: 12)),
              Container(
          ...
          Center(
            child: Container(
              ...
              child: Row(
                children: [
                  Expanded(child: _buildInfoColumn('درجة اللون', 'A2')),
                  const VerticalDivider(width: 1, color: Color(0xFFF3F4F6)),
                  Expanded(child: _buildInfoColumn('نوع المادة', 'Zirconia')),
                ],
              ),
            ),
          ),
