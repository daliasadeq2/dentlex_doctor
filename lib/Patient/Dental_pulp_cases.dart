...
class DentalPulpCasesScreen extends StatefulWidget {
  const DentalPulpCasesScreen({super.key});

  @override
  State<DentalPulpCasesScreen> createState() => _DentalPulpCasesScreenState();
}

class _DentalPulpCasesScreenState extends State<DentalPulpCasesScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = false;
  String activeFilter = 'حالات اللب السني';

  final LayerLink _pulpLayerLink = LayerLink();
  OverlayEntry? _pulpOverlayEntry;
  bool _isPulpMenuOpen = false;

  void _togglePulpMenu() {
    if (_isPulpMenuOpen) {
      _closePulpMenu();
    } else {
      _openPulpMenu();
  ...
  void _openPulpMenu() {
    setState(() {
      _isPulpMenuOpen = true;
      activeFilter = 'حالات اللب السني';
    });
    _pulpOverlayEntry = _createPulpOverlayEntry();
    Overlay.of(context).insert(_pulpOverlayEntry!);
  }

  void _closePulpMenu() {
    if (_pulpOverlayEntry != null) {
      _pulpOverlayEntry!.remove();
      _pulpOverlayEntry = null;
      setState(() {
        _isPulpMenuOpen = false;
      });
    }
  }

  OverlayEntry _createPulpOverlayEntry() {
    final List<String> pulpCases = [
      'التهاب لب قابل للعكس',
      'التهاب لب غير قابل للعكس',
      'لب نخر',
      'امتصاص داخلي',
      'تكلس اللب',
      'فرط تنسج اللب',
                          ...
                          builder: (context, setPopupState) {
                            ...
                            return InkWell(
                              onTap: () {
                                ...
                                setPopupState(() {});
                                _pulpOverlayEntry?.markNeedsBuild();
                              },
                              child: Container(
        ...
        child: Row(
          children: [
            CompositedTransformTarget(
              link: _pulpLayerLink,
              child: _buildPulpFilterButton('حالات اللب السني', activeFilter.contains('اللب السني') || _isPulpMenuOpen || [
                'التهاب لب قابل للعكس',
                'التهاب لب غير قابل للعكس',
                'لب نخر',
                'امتصاص داخلي',
  ...
  Widget _buildPulpFilterButton(String label, bool isActive) {
    return GestureDetector(
      onTap: _togglePulpMenu,
      child: Container(
        height: 40,
