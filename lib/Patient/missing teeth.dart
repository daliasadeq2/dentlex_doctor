...
class MissingTeethScreen extends StatefulWidget {
  const MissingTeethScreen({super.key});

  @override
  State<MissingTeethScreen> createState() => _MissingTeethScreenState();
}

class _MissingTeethScreenState extends State<MissingTeethScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = false;
  String activeFilter = 'الأسنان المفقودة';

  final LayerLink _missingTeethLayerLink = LayerLink();
  OverlayEntry? _missingTeethOverlayEntry;
  bool _isMissingTeethMenuOpen = false;

  void _toggleMissingTeethMenu() {
    if (_isMissingTeethMenuOpen) {
      _closeMissingTeethMenu();
    } else {
      _openMissingTeethMenu();
    }
  }

  void _openMissingTeethMenu() {
    setState(() {
      _isMissingTeethMenuOpen = true;
      activeFilter = 'الأسنان المفقودة';
    });
    _missingTeethOverlayEntry = _createMissingTeethOverlayEntry();
    Overlay.of(context).insert(_missingTeethOverlayEntry!);
  }

  void _closeMissingTeethMenu() {
    if (_missingTeethOverlayEntry != null) {
      _missingTeethOverlayEntry!.remove();
      _missingTeethOverlayEntry = null;
      setState(() {
        _isMissingTeethMenuOpen = false;
      });
    }
  }

  OverlayEntry _createMissingTeethOverlayEntry() {
    final List<String> cases = [
      'مفقود خلقياً',
      'مخلوع',
      'مفقود نتيجة انخلاع كامل',
      'غير بازغ',
      'مفقود بسبب إصابة أو صدمة',
    ];
                          ...
                          builder: (context, setPopupState) {
                            ...
                            return InkWell(
                              onTap: () {
                                ...
                                setPopupState(() {});
                                _missingTeethOverlayEntry?.markNeedsBuild();
                              },
                              child: Container(
        ...
        child: Row(
          children: [
            ...
            CompositedTransformTarget(
              link: _missingTeethLayerLink,
              child: _buildMissingTeethFilterButton('الأسنان المفقودة', activeFilter.contains('مفقود') || activeFilter == 'مخلوع' || activeFilter == 'غير بازغ' || _isMissingTeethMenuOpen),
            ),
            const SizedBox(width: 8),
            _buildFilterChip(
              'الأسنان المطمورة', 
            ...
            const SizedBox(width: 8),
            _buildFilterChip('مشاكل الاطباق', activeFilter == 'مشااقل الاطباق'),
            const SizedBox(width: 8),
  ...
  Widget _buildMissingTeethFilterButton(String label, bool isActive) {
    return GestureDetector(
      onTap: _toggleMissingTeethMenu,
      child: Container(
        height: 40,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          const Text(
            'المريض استجاب بشكل جيد للإجراء اليوم بدون أي مضاعفات تُذكر.',
