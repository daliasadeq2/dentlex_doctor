...
class MeasurementsDentalImplantsScreen extends StatefulWidget {
  const MeasurementsDentalImplantsScreen({super.key});

  @override
  State<MeasurementsDentalImplantsScreen> createState() => _MeasurementsDentalImplantsScreenState();
}

class _MeasurementsDentalImplantsScreenState extends State<MeasurementsDentalImplantsScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = true;
  String activeFilter = 'زراعة الأسنان';
  ...
  Widget _buildActionTabs() {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: GestureDetector(onTap: () => setState(() => isTherapeutic = true), child: _buildActionTab('الحالات العلاجية', isTherapeutic))),
          Expanded(child: GestureDetector(onTap: () => setState(() => isTherapeutic = false), child: _buildActionTab('الحالات المرضية', !isTherapeutic))),
        ],
      ),
    );
  }
  ...
  Widget _buildFilterChip(String label, bool isActive) {
    return GestureDetector(
      onTap: () {
        ...
        } else if (label == 'التيجان والجسور') {
           Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const CracksAndFracturesScreen()));
      ...
      child: Row(
        children: [
          GestureDetector(
            ...
            child: _buildDetailTab('الحالة', false),
          ),
          const SizedBox(width: 12),
          GestureDetector(
            onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const AbscessScreen())),
            child: _buildDetailTab('وصفة', false),
          ),
          const SizedBox(width: 12),
          GestureDetector(
            onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const MissingTeethScreen())),
            child: _buildDetailTab('ملاحظات', false),
          ),
          const SizedBox(width: 12),
          GestureDetector(
            onTap: () {
              if (activeFilter == 'التيجان والجسور') {
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const CracksAndFracturesScreen()));
              } else if (activeFilter == 'زراعة الأسنان') {
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const MeasurementsDentalImplantsScreen()));
              } else {
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const ImpactedTeethScreen()));
              }
            },
            child: _buildDetailTab('قياسات', true),
          ),
          const SizedBox(width: 12),
          _buildDetailTab('مختبر', false),
          const SizedBox(width: 12),
          GestureDetector(
            onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const ReminderDiseaseScreen())),
            child: _buildDetailTab('تذكير', false),
          ),
          const SizedBox(width: 12),
