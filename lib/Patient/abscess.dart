...
class AbscessScreen extends StatefulWidget {
  const AbscessScreen({super.key});

  @override
  State<AbscessScreen> createState() => _AbscessScreenState();
}

class _AbscessScreenState extends State<AbscessScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = false;
  String activeFilter = 'الخراج';

  final LayerLink _abscessLayerLink = LayerLink();
  OverlayEntry? _abscessOverlayEntry;
  bool _isAbscessMenuOpen = false;

  GestureTapCallback? get _togglePulpMenu => null;

  void _toggleAbscessMenu() {
    if (_isAbscessMenuOpen) {
      _closeAbscessMenu();
    } else {
      _openAbscessMenu();
    }
  ...
  void _openAbscessMenu() {
    setState(() {
      _isAbscessMenuOpen = true;
      activeFilter = 'الخراج';
    });
    _abscessOverlayEntry = _createAbscessOverlayEntry();
    Overlay.of(context).insert(_abscessOverlayEntry!);
  }

  void _closeAbscessMenu() {
    if (_abscessOverlayEntry != null) {
      _abscessOverlayEntry!.remove();
      _abscessOverlayEntry = null;
      setState(() {
        _isAbscessMenuOpen = false;
      });
    }
  }

  OverlayEntry _createAbscessOverlayEntry() {
    final List<String> abscessCases = [
      'خراج حول ذروة السن',
      'خراج لثوي',
      'خراج ذروي حاد',
      'خراج ذروي مزمن',
                ...
                child: Directionality(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: abscessCases.map((choice) {
                          ...
                          builder: (context, setPopupState) {
                            ...
                            return InkWell(
                              onTap: () {
                                ...
                                setPopupState(() {});
                                _abscessOverlayEntry?.markNeedsBuild();
                              },
                              child: Container(
        ...
        child: Row(
          children: [
            ...
            CompositedTransformTarget(
              link: _abscessLayerLink,
              child: _buildAbscessFilterButton('الخراج', activeFilter.contains('الخراج') || _isAbscessMenuOpen || [
                'خراج حول ذروة السن',
                'خراج لثوي',
                'خراج ذروي حاد',
                'خراج ذروي مزمن',
            ...
            const SizedBox(width: 8),
            _buildFilterChip('الأسنان المفقودة', activeFilter == 'الأسنان المفقودة', onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const MissingTeethScreen()))),
            const SizedBox(width: 8),
            _buildFilterChip(
              'الأسنان المطمورة', 
  ...
  Widget _buildAbscessFilterButton(String label, bool isActive) {
    return GestureDetector(
      onTap: _toggleAbscessMenu,
      child: Container(
        height: 40,
  ...
  // تم تعديل الدالة لتستقبل الوظيفة onTap اختيارياً منعاً للتعارض
