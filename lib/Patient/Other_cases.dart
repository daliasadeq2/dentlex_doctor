...
class OtherCasesScreen extends StatefulWidget {
  const OtherCasesScreen({super.key});

  @override
  State<OtherCasesScreen> createState() => _OtherCasesScreenState();
}

class _OtherCasesScreenState extends State<OtherCasesScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  bool isTherapeutic = false;
  String activeFilter = 'حالات اخرى';

  final LayerLink _otherCasesLayerLink = LayerLink();
  OverlayEntry? _otherCasesOverlayEntry;
  bool _isOtherCasesMenuOpen = false;

  void _toggleOtherCasesMenu() {
    if (_isOtherCasesMenuOpen) {
      _closeOtherCasesMenu();
    } else {
      _openOtherCasesMenu();
    }
  }

  void _openOtherCasesMenu() {
    setState(() {
      _isOtherCasesMenuOpen = true;
      activeFilter = 'حالات اخرى';
    });
    _otherCasesOverlayEntry = _createOtherCasesOverlayEntry();
    Overlay.of(context).insert(_otherCasesOverlayEntry!);
  }

  void _closeOtherCasesMenu() {
    if (_otherCasesOverlayEntry != null) {
      _otherCasesOverlayEntry!.remove();
      _otherCasesOverlayEntry = null;
      setState(() {
        _isOtherCasesMenuOpen = false;
      });
    }
  }

  OverlayEntry _createOtherCasesOverlayEntry() {
    // العناصر المأخوذة من الصورة المرفقة لقائمة حالات أخرى
    final List<String> otherCasesList = [
      'امتداد الحالة إلى الجيب الأنفي',
      'قناة جذرية متكلسة',
      'امتصاص (خارجي/داخلي)',
      'سن ملتحم بالعظم',
      'أسنان زائدة',
      'شفافية شعاعية حول الذروة',
      'فرط تكون الملاط',
      'انحناء أو تشوه الجذر',
      'تقوس الجذر',
      'تقارب الجذور',
      'تبدل مواضع الأسنان',
      'زيادة أو نقص في حشو قناة الجذ',
    ];
                  ...
                  child: Container(
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: otherCasesList.map((choice) {
                            ...
                            builder: (context, setPopupState) {
                              ...
                              return InkWell(
                                onTap: () {
                                  ...
                                  setPopupState(() {});
                                  _otherCasesOverlayEntry?.markNeedsBuild();
                                },
                                child: Container(
        ...
        child: Row(
          children: [
            ...
            _buildFilterChip(
              ...
              onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const DishProblemsScreen())),
            ),
            const SizedBox(width: 8),
            CompositedTransformTarget(
              link: _otherCasesLayerLink,
              child: _buildOtherCasesFilterButton('حالات اخرى', activeFilter == 'حالات اخرى' || _isOtherCasesMenuOpen || [
                'امتداد الحالة إلى الجيب الأنفي',
                'قناة جذرية متكلسة',
                'امتصاص (خارجي/داخلي)',
                'سن ملتحم بالعظم',
  ...
  Widget _buildOtherCasesFilterButton(String label, bool isActive) {
    return GestureDetector(
      onTap: _toggleOtherCasesMenu,
      child: Container(
        height: 40,
